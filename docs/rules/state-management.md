...
- `src/apis` 直下に本番用APIラッパーや業務ロジックを置かない
- HTTP statusの判定、状態遷移、ユーザー向けエラーは状態を所有するstore / Server Componentで扱う
- 表示用のderived valueはselectorまたはrender時に計算する
- 再利用可能な純粋変換はfeature配下または `src/lib` に置く
...
- feature / page専用storeとして、原則 `src/stores/<feature>` 配下へ配置する
- UIコンポーネント固有の一時状態ではなく、feature / page内で共有されるAPI状態・共有client stateを対象とする
- component local stateで十分な状態は `src/stores` に移さない
...
- storeを読むhook / adapterはselectorを受け取り、購読範囲を必要なstate / actionへ限定する
- 複数値をobjectで返すselectorは、shallow比較等で同値objectによる不要な再レンダーを防ぐ
- store全体の購読は、そのconsumerが全フィールドを描画に利用し、更新頻度と再レンダー影響を説明できる場合に限る
- 高頻度に変わるpending、入力、通知等を利用しないconsumerまで再レンダーさせない
- action名はUIイベントではなく、状態遷移の意図が分かる名前にする

### Providerとselectorの利用

同じfeature storeを複数の表示領域で利用する場合は、Providerでinstanceを1回だけ生成し、各領域が必要な値だけをselectorで購読する。
...
複数値をまとめる場合は、毎回新しいobjectを返すだけのselectorにせず、shallow比較を適用する。

```tsx
const formState = useTodos(
  useShallow((state) => ({
...
Providerはstore初期値と自動取得の開始責務を持つ。selector hookはstore生成やprops同期を行わず、購読だけを担当する。
...
- `src/apis` 直下へ本番用APIラッパー、レスポンス変換、状態遷移、画面固有エラー処理を追加する
- Orval共通mutatorへドメイン固有ロジックを追加する
...
- [ ] 本番コードが `src/apis/generated` を直接利用し、`src/apis` 直下に本番用ラッパーを追加していない
- [ ] API状態とUI状態の責務が分離されている
- [ ] Zustandの採用理由とstoreのfeature / page境界が説明できる
- [ ] store instanceがSSR request、Story、testごとに独立している
- [ ] store consumerがselectorで必要なstate / actionだけを購読している
- [ ] 未選択状態の更新で不要な再レンダーが発生しないことを必要に応じてtestで保証している
